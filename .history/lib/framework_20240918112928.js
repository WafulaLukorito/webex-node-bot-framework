 ...
 */
class Framework extends EventEmitter2 {
  constructor(options) {
    super();
     ...
     * @property {number}[maxStartupSpaces] - If specified, the maximum number of spaces with our bot that the framework will discover during startup.  
      ...
      * @property {boolean}[supressDirectSpaceSpawnFailures=true] - When a bot has a 1-1 space with a webex user who is no longer active the bot will fail to spawn.  By default these error messages are supressed.  Set to false to see them. 
      * @property {string} [messageFormat=text] - Default Webex message format to use with bot.say().
      * @property {string} [profileMsgProcessingTime=false] - Set to true to profile time spent in franmework and hears() callbacks per message.
    ...
    // If requested, initialize message process time profiling
    if (this.options.profileMsgProcessingTime) {
      this.numMessages = 0;
      this.cumMsgProcessTime = 0;
    }
    ...
    } else if ('maxStartupSpaces' in options) {
    ...
    this.supressDirectSpaceSpawnFailures = 
      'supressDirectSpaceSpawnFailures' in options ? options.supressDirectSpaceSpawnFailures : true;
...
Framework.prototype.stop = function () {
  ...
  if (this.active) {
    ...
    // Display msg processing profiling info
    if (this.options.profileMsgProcessingTime && this.numMessages) {
      console.log(`${this.numMessages} messages processed by bot since startup.`);
      console.log('Average framework and hears() processing time per message was %i msecs', this.cumMsgProcessTime / this.numMessages);
    }
...
Framework.prototype.initialize = function () {
  ...
  if (this.maxStartupSpaces != 0) {
    ...
    if (('maxStartupSpaces' in this) && (this.maxStartupSpaces < 1000)) {
...
Framework.prototype.spawn = function (membership, actorId) {
  ...
  return this.webex.rooms.get(roomId)
    ...
    .catch(err => {
      if ((!this.supressDirectSpaceSpawnFailures) || 
        (err.message != 'Not spawning bot in 1-1 space where bot is the only remaining member.')) {
