 ...
 *                                  Passing a message object ensures that the reply will succeed even if the parent message is also a reply in a thread.
 * @param {String|Object} message - Message to send to room. This can be a simple string, or a message object for advanced use.
 * @param {String} [format=text] - Set message format. Valid options are 'text' or 'markdown'. Default format is used if not set.
 * @returns {Promise.<Message>}
 *
 * @example
 * // Simple example
...
Bot.prototype.dm = function (person, format, message) {
  ...
  if ('membershipRules' in this.framework) {
    membershipPromise = this.framework.membershipRules.isNewPersonAllowed(person);
  } else {
    membershipPromise = when.resolve(true);
  }

  return when(membershipPromise).then((isAllowed) => {
    if (isAllowed) {
      // parse function args and check for wierd behavior of 
      // arguments object when function is called in a function
      var args = Array.prototype.slice.call(arguments);
...
Bot.prototype.dmCard = function (person, cardJson, fallbackText) {
  let messageObj = buildCardMsgObj(cardJson, 
    ...
    // membership rules checked in this.dm()
